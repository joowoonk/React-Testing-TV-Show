...
test("Allows to console", async () => {
  mockFetchShow.mockResolvedValueOnce(episodesSeasonThree);

  const { queryAllByTestId } = render(<App />);

  // const value = getByDisplayValue(/Select a season/i);

  await waitFor(() => {
    // expect(userEvent.click(<Dropdown />));

    expect(queryAllByTestId(/dropDown/i)).toHaveLength(0);
  });
});
